...
class Settings(BaseSettings):
    ...
    # Повторная отправка SOS data-пуша через N секунд (0 = выключено).
    # Зачем: приёмник firebase_messaging решает «приложение на экране?» по
    # importance процесса, а Android опускает importance лениво — свёрнутое
    # секунды назад приложение ещё числится «передним». Такой пуш уходит в
    # LiveData под наблюдателя активности, которая свёрнута, и повисает там
    # непоказанным: ни сирены, ни уведомления (Samsung A51, 2026-08-13; оффер
    # через 2 с после Home терялся, отмена через 14 с — доходила). Ко второй
    # отправке importance уже опущен → пуш идёт фоновым путём → сирена. Дубль
    # для клиента безопасен: id уведомления фиксированный (замена, не второе),
    # внутриэкранная сирена защищена флагом от повторного старта.
    sos_push_resend_delay: float = 0.0

...
class EmergencyCall(Base):
    ...
    status = Column(Enum(CallStatus, native_enum=False, values_callable=lambda x: [e.value for e in x]), default=CallStatus.CREATED)
...
class CallStatusHistory(Base):
    ...
    status = Column(Enum(CallStatus, native_enum=False, values_callable=lambda x: [e.value for e in x]), nullable=False)
